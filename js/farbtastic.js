...
$._farbtastic = function (container, options) {
  ...
  fb.drawCircle = function () {
    ...
    for (let i = 0; i <= n; ++i) {
      let d1 =0;
      let d2 = i / n;
      let angle2 = d2 * Math.PI * 2;
          // Endpoints
      let x1 = Math.sin(angle1);
      let y1 = -Math.cos(angle1);
      let x2 = Math.sin(angle2);
      let y2 = -Math.cos(angle2);
        ...
        let color2 = fb.pack(fb.HSLToRGB([d2, 1, 0.5]));
      if (i > 0) {
        if ($.browser.msie) {
          // IE's gradient calculations mess up the colors. Correct along the diagonals.

          let corr = (1 + Math.min(Math.abs(Math.tan(angle1)), Math.abs(Math.tan(Math.PI / 2 - angle1)))) / n;
          color1 = fb.pack(fb.HSLToRGB([d1 - 0.15 * corr, 1, 0.5]));
          color2 = fb.pack(fb.HSLToRGB([d2 + 0.15 * corr, 1, 0.5]));
          // Create gradient fill between the endpoints.
          let grad = m.createLinearGradient(x1, y1, x2, y2);
          grad.addColorStop(0, color1);
          grad.addColorStop(1, color2);
          m.fillStyle = grad;
          // Draw quadratic curve segment as a fill.
          let r1 = (r + w / 2) / r, r2 = (r - w / 2) / r; // inner/outer radius.
          m.beginPath();
  ...
  fb.drawMask = function () {
    ...
    if (fb.ctxMask.getImageData) {
      ...
      let frame = ctx.getImageData(0, 0, sz + 1, sz + 1);
    ...
    // Method #3: vertical DXImageTransform gradient strips (IE).
      ...
      let cache_last, cache, w = 6; // Each strip is 6 pixels wide.
      let sizex = Math.floor(size / w);
